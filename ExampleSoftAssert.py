...
class ExampleSoftAssert(SoftAssert):
    ...
    def test_soft_assert_api(self):
        ...
        :return:
        """
        print("Running test_soft_assert_api")

        response = requests.get("https://www.google.com/search?q=python")

        # success expected
        SoftAssert.soft_assert(self, self.assertEqual, response.status_code, 200,
                               f"Request failed with status {response.status_code}")

        headerCount = response.content.decode().count("h1")
        # assertion error expected
        SoftAssert.soft_assert(self, self.assertGreater, headerCount, 10,
                               f"Found {headerCount} headers, but expected 10")
    ...
    def test_soft_assert_webdriver(self):
        ...
        :return:
        """
        print("Running test_soft_assert_webdriver")
        ...
        SoftAssert.soft_assert(self, self.assertGreater, headerCount, 10,
                               msg=f"Found {headerCount} headers, but expected 10")
